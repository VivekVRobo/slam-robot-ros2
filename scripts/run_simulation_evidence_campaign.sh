...
ros2 topic list > "$RUN_DIR/topics-final.txt" 2>/dev/null || true
...
for required in \
  trajectory.csv trajectory-metrics.json loop-metrics.json resource-profile.json map.pgm map.yaml; do
...
The manifest and hashes establish provenance for the captured simulation artifacts.
